...
//	const S_CHAR input_file[] = "cell_2.oas";
	...
	WriteOasis2Text(oasis_arr, oasis_length, input_file, output_file);
...
//	printf("oasis.remain_last_pos = %u\n", oasis.remain_last_pos);
...
//		printf("oasis.unit_begin_pos + oasis.unit_length = %u\n", oasis.unit_begin_pos + oasis.unit_length);

		// if table-offsets at START
		if(i == oasis.offset_flag_pos)
		{
			oasis.offset_flag = (BOOL) ch;
...
//		printf("oasis.remain_last_pos = %u\n", oasis.remain_last_pos);
		...
		if((i >= oasis.next_rec_begin_pos) && (i < oasis.remain_last_pos))
		{
			...
			DumpRemainRecord(oasis.next_rec_begin_pos, oasis_arr, cell, oasis.unit, oasis.next_rec_length, write_file);
...
VOID DumpRemainRecord(U_INT rec_begin_pos_in, S_INT oasis_arr[], CELL & cell_out, DOUBLE unit_in, U_INT &rec_length, FILE *write_file)
{
	switch(oasis_arr[rec_begin_pos_in])
	{
		...
		case PLACEMENT17_ID:
			DumpPlacement(rec_begin_pos_in, oasis_arr, rec_length, write_file);
			break;

		case TEXT_ID:
			DumpText(rec_begin_pos_in, oasis_arr, rec_length, write_file);
			break;

		case RECTANGLE_ID:
			DumpRectangle(rec_begin_pos_in, oasis_arr, cell_out, unit_in, rec_length, write_file);
			break;

		case POLYGON_ID:
			DumpPolygon(rec_begin_pos_in, oasis_arr, cell_out, unit_in, rec_length, write_file);
			break;
