...
VOID DumpCellName(U_INT rec_begin_pos_in, S_INT oasis_arr[], U_INT &rec_length_out, FILE *write_file)
{
	...
	U_INT byte_num;
	U_INT byte_begin_pos = rec_begin_pos_in + CELLNAME_ID_LENGTH;

	cellname.id = (U_CHAR) oasis_arr[rec_begin_pos_in];

	/* ‘3’ cellname-string */
	if(cellname.id == CELLNAME3_ID)
	{
		fprintf(write_file, "\n\n%09d\tCELLNAME id=%u cellname=\"", rec_begin_pos_in, cellname.id);
		byte_num = GetIntegerByteNum(oasis_arr, byte_begin_pos);
		cellname.string_length = GetOasisUINT(byte_num, oasis_arr, byte_begin_pos);
		byte_begin_pos += (byte_num + cellname.string_length);

		for(U_INT i = 0; i < cellname.string_length; i++)
		{
	...
	{
		fputs("[CELLNAME ID4] do not implement yet\n", write_file);
	}
